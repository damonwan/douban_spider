...
def get_last_url(conn):
    cursor = conn.cursor()
    cursor.execute('select * from access_log order by id desc limit 1')
    return cursor.fetchone()
    
def save_access_log(conn, url):
    cursor = conn.cursor()
    cursor.execute('insert into access_log (url) values (%s)', [url,])
    conn.commit()
    cursor.close()
    
def save_failed_log(conn, url):
    cursor = conn.cursor()
    cursor.execute('insert into failed_log (url) values (%s)', [url,])
    conn.commit()
    cursor.close()
